...
for sa = 1:length(ScalpArea)
    ...
    for subcp = 1:length(subcouple)
        ...
        for iband = 1:size(Erp(1).data4D,3)
            ...
            plot(Xf,C3,'Color',colors{3},'LineStyle','-','LineWidth',2)
            ...
            plot(Xf,p13_any*100-99.8+v(3)*.95,'k^','MarkerSize',5) %[.3 .4 .2]
            plot(Xf,p23_any*100-100+v(3)*.95,'kx','MarkerSize',7) %[0.8 0.4 0]
            axis(v) % restore axis limits to hide non-significant points
            set(gcf,'PaperPositionMode','auto')
            set(gcf, 'WindowState', 'maximized');
            legend(legendLab{1:3},'')


            name1 = strcat('ERP_in_', bands{iband});
            % plot saving params
            save_dir1 ='D:\main_scriptNSA\Older_and_Proof\Sound_pipeline\11_06\ERP_analysis\N400Plot\BaselineRemoved\NEW_FIGURE\Confronto_a_3\';
