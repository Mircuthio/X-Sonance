function [ResultsTable, ResultsStruct] = ...
    collect_fbcsp_loso_results(rootDir, outputFile)

% COLLECT_FBCSP_LOSO_RESULTS
%
% Scans all case folders recursively, loads .mat result files,
% and extracts LOSO classifier summaries into a table and struct array.
%
% INPUT
...
for iFile = 1:numel(matFiles)
    ...
    try
        ...
        if ~isfield(loadedData, 'Results_LOSO')
            warning('Skipped file without Results_LOSO: %s', filePath);
            continue
        end

        Results_LOSO = loadedData.Results_LOSO;
        ...
        classifierNames = fieldnames(Results_LOSO);
        ...
        for iClassifier = 1:numel(classifierNames)
            ...
            classifierResult = Results_LOSO.(classifierName);
...
if isempty(ResultsStruct)
    ResultsTable = table();
    warning('No valid LOSO results were extracted.');
    return
end
