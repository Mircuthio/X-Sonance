function OUT = plot_erp_waveforms(ERP, cfg)
% PLOT_ERP_WAVEFORMS
%
% Visualizes ERP waveforms from an ERP structure.
%
% Supports:
%   - Single subject ERP
%   - ROI ERP
%   - Group ERP
%
% Optional cfg fields:
%
%   cond_idx
%   line_colors
%   patch_colors
%   line_width
%   line_width_map
%   patch_alpha
%   smooth_plot
%   smooth_window
%   show_zero
%   show_error
%   error_data
%   title_str
%   ylabel_str
%   xlim
%   ylim
%   save_path

%% ============================================================
% DEFAULTS
%% ============================================================

if nargin < 2
    cfg = struct();
end

if ~isfield(cfg,'cond_idx')
    cfg.cond_idx = 1:numel(ERP.conditions);
end

if ~isfield(cfg,'line_colors')
    cfg.line_colors = lines(numel(cfg.cond_idx));
end

if ~isfield(cfg,'patch_colors')
    cfg.patch_colors = cfg.line_colors;
end

if ~isfield(cfg,'show_zero')
    cfg.show_zero = true;
end

if ~isfield(cfg,'show_error')
    cfg.show_error = true;
end

if ~isfield(cfg,'title_str')
    cfg.title_str = 'ERP';
end

if ~isfield(cfg,'ylabel_str')
    cfg.ylabel_str = '\muV';
end

if ~isfield(cfg,'xlim')
    cfg.xlim = [];
end

if ~isfield(cfg,'ylim')
    cfg.ylim = [];
end

if ~isfield(cfg,'save_path')
    cfg.save_path = '';
end

if ~isfield(cfg,'smooth_plot')
    cfg.smooth_plot = false;
end

if ~isfield(cfg,'smooth_window')
    cfg.smooth_window = 5;
end

if ~isfield(cfg,'patch_alpha')
    cfg.patch_alpha = 0.20;
end

if ~isfield(cfg,'line_width')
    cfg.line_width = 2;
end

if ~isfield(cfg,'line_width_map')
    cfg.line_width_map = [];
end

%% ============================================================
% CHECKS
%% ============================================================

assert(isfield(ERP,'time_erp'),...
    'ERP.time_erp missing');

assert(isfield(ERP,'conditions'),...
    'ERP.conditions missing');

assert(isfield(ERP,'grand_avg'),...
    'ERP.grand_avg missing');

if ~isempty(cfg.line_width_map)

    assert( ...
        numel(cfg.line_width_map) == numel(cfg.cond_idx), ...
        'line_width_map size mismatch');

end

%% ============================================================
% DATA
%% ============================================================

tt = ERP.time_erp(:).';

cond_idx = cfg.cond_idx(:).';

nCond = numel(cond_idx);

%% ============================================================
% FIGURE
%% ============================================================

f = figure( ...
    'Color','w',...
    'Position',[100 100 1100 500]);

hold on

hLines = gobjects(nCond,1);

%% ============================================================
% LOOP CONDITIONS
%% ============================================================

for i = 1:nCond

    cidx = cond_idx(i);

    m = squeeze(ERP.grand_avg(cidx,:));

    if cfg.smooth_plot
        m = movmean(m,cfg.smooth_window);
    end

    %% --------------------------------------------------------
    % ERROR REGION
    %% --------------------------------------------------------

    if cfg.show_error && ~isempty(cfg.error_data)

        err = squeeze(cfg.error_data(cidx,:));

        if cfg.smooth_plot
            err = movmean(err,cfg.smooth_window);
        end

        xPatch = [tt fliplr(tt)];
        yPatch = [(m-err) fliplr(m+err)];

        patch( ...
            xPatch,...
            yPatch,...
            cfg.patch_colors(i,:),...
            'FaceAlpha',cfg.patch_alpha,...
            'EdgeColor','none');

    end

    %% --------------------------------------------------------
    % LINE WIDTH
    %% --------------------------------------------------------

    lw = cfg.line_width;

    if ~isempty(cfg.line_width_map)
        lw = cfg.line_width_map(i);
    end

    %% --------------------------------------------------------
    % ERP LINE
    %% --------------------------------------------------------

    hLines(i) = plot( ...
        tt,...
        m,...
        'Color',cfg.line_colors(i,:),...
        'LineWidth',lw);

end

%% ============================================================
% REFERENCE LINES
%% ============================================================

if cfg.show_zero

    xline(0,...
        'k--',...
        'LineWidth',1);

    yline(0,...
        'k:',...
        'LineWidth',1);

end

%% ============================================================
% AXES
%% ============================================================

if ~isempty(cfg.xlim)
    xlim(cfg.xlim);
end

if ~isempty(cfg.ylim)
    ylim(cfg.ylim);
end

if max(abs(tt)) < 10
    xlabel('Time (s)');
else
    xlabel('Time (ms)');
end

ylabel(cfg.ylabel_str);

title(cfg.title_str);

legend( ...
    hLines,...
    ERP.conditions(cond_idx),...
    'Location','best');

set(gca,...
    'FontSize',11,...
    'Box','off');

set(gca,'Layer','top');

%% ============================================================
% SAVE
%% ============================================================

if ~isempty(cfg.save_path)

    [p,~,~] = fileparts(cfg.save_path);

    if ~isempty(p) && ~exist(p,'dir')
        mkdir(p);
    end

    exportgraphics( ...
        f,...
        cfg.save_path,...
        'Resolution',300);

end

%% ============================================================
% OUTPUT
%% ============================================================

OUT = struct();

OUT.figure     = f;
OUT.handles    = hLines;
OUT.time       = tt;
OUT.conditions = ERP.conditions(cond_idx);
OUT.cfg        = cfg;

end